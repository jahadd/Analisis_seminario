...
resultados_links <- map_df(categorias, \(enlace_categoria) {
  ...
  tryCatch({
    ...
    inicio <- Sys.time()
    chrome$Page$navigate(enlace_categoria) # navegar a la página
...
# guardar ----
# dir.create("resultados/adnradio/", showWarnings = F)
#resultados_adnradio

readr::write_rds(resultados_adnradio,
                 # glue("resultados/adnradio/adnradio_cron_{rng()}_{lubridate::today()}.rds")
                 ruta_resultado("adnradio", hist)
)
